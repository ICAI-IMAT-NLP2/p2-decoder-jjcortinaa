...
class AttentionHead(nn.Module):
    ...
    def __init__(self, d_model: int, d_k: int, d_q: int, d_v: int):
        ...
        self.wq = nn.Linear(in_features=d_model,out_features=d_q, bias=True)
        self.wk = nn.Linear(in_features=d_model,out_features=d_k, bias=True)
        self.wv = nn.Linear(in_features=d_model,out_features=d_v, bias=True)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        vec_dim = d_model//num_attention_heads

        self.heads = nn.ModuleList([AttentionHead(d_model,vec_dim,vec_dim,vec_dim) for i in range(num_attention_heads)])
        self.output_linear = nn.Linear(in_features=num_attention_heads*vec_dim, out_features= d_model, bias = True)
    ...
    def forward(self, hidden_state, mask=None):
        ...
        if self.d_model % self.num_attention_heads != 0:
            raise RuntimeError(
                f"d_model={self.d_model} no es divisible por num_heads={self.num_attention_heads}"
            )
        result = [head(hidden_state,mask) for head in self.heads]
        concat = torch.cat(result, dim=-1)
...
class FeedForward(nn.Module):
    ...
    def __init__(self, d_model: int, intermediate_size: int):
        ...
        self.linear_1 = nn.Linear(in_features=d_model, out_features=intermediate_size, bias = True)
        self.linear_2 = nn.Linear(in_features=intermediate_size, out_features=d_model, bias = True)
        self.gelu = nn.GELU()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        activation = self.gelu(out_linear_1)
        out_linear_2 = self.linear_2(activation)
        return out_linear_2
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        multihead_attention = self.self_attention(hidden_state,mask)
        multihead_attention+=x
        
        # Apply layer normalization and then apply feed-forward network

        layer_norm_2 = self.layer_norm_2(multihead_attention)
        x = self.feed_forward(layer_norm_2) + multihead_attention
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.layers:  # aplicar secuencialmente
            x = layer(x,mask)
        return x
...
class TransformerForLanguageModeling(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        x = self.transformer_decoder(input_ids)
